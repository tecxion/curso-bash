...
*   *Excepción*: No se detendrá si el comando está dentro de una estructura de control (como un `if` o `while`).
...
4. Escribe un caso donde quieras usar `set -e` pero necesites ejecutar un comando que *sabes* que puede fallar (por ejemplo, comprobar si un puerto está abierto) sin que detenga todo el script. Investiga cómo evitar la detención usando el operador lógico OR `|| true` o un bloque condicional.
